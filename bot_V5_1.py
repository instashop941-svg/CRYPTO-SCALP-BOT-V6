...
MIN_SL_CLEARANCE_PCT = float(os.getenv("MIN_SL_CLEARANCE_PCT", "0.0010"))
...
def detect(symbol):
    ...
    # V5.2 structural SL: place the invalidation beyond the deepest structural
    # point (sweep + POI + IMB), then add a dynamic buffer using recent 1m ATR.
    # This is intentionally NOT a fixed distance from entry.
    trs=[]
    for j in range(max(1, len(d)-15), len(d)):
        prev_close=float(d[j-1][4])
        high=float(d[j][2]); low=float(d[j][3])
        trs.append(max(high-low, abs(high-prev_close), abs(low-prev_close)))
    atr=median(trs) if trs else 0.0
    def structural_buffer(level):
        return max(level*SL_BUFFER_PCT, atr*SL_ATR_MULT, entry*MIN_SL_CLEARANCE_PCT)

    if side=="LONG":
        invalid=min(sweep_extreme, poi_low, fvg_low)
        buffer=structural_buffer(invalid)
        sl=invalid-buffer
        room=TP2_PCT
        risk=(entry-sl)/entry
        if sl >= invalid or sl >= poi_low:
            print(f"[FILTER SL] {symbol} LONG SL not beyond structural invalidation", flush=True)
            return None
    else:
        ...
        room=TP2_PCT
        risk=(sl-entry)/entry
        if sl <= invalid or sl <= poi_high:
            print(f"[FILTER SL] {symbol} SHORT SL not beyond structural invalidation", flush=True)
            return None
